 ...
 * - Component library CRUD operations (scoped to active project)
 * - UI state management
...
interface ComponentActions {
  ...
  // Component Library Management (scoped to active project)
  addComponent: () => void;
...
export const useComponentStore = create<ComponentState & ComponentActions & ComputedState>((set, get) => {
  ...
  // Create default project with the default component
  ...
  return {
    // Initial State with Project layer
    projects: {
...
`,
  ...
  theme: {
    ...
    // New project-aware computed getters
    getActiveProject: () => {
    ...
    get componentAst() {
      const comp = get().getActiveComponent();
      return comp?.componentAst || null;
    },
    get componentPreviewAst() {
      const comp = get().getActiveComponent();
      return comp?.componentPreviewAst || null;
    },
    get history() {
      const comp = get().getActiveComponent();
      return comp?.history || [{ ast: null, preview: null }];
    },
    get historyIndex() {
      const comp = get().getActiveComponent();
      return comp?.historyIndex || 0;
    },
    get propsJson() {
      const comp = get().getActiveComponent();
      return comp?.propsJson || '{}';
    },
    get dependencies() {
      const comp = get().getActiveComponent();
    ...
    get wrapperCode() {
      const comp = get().getActiveComponent();
      return comp?.wrapperCode || initialWrapperCode;
    },
    get originalCode() {
      const comp = get().getActiveComponent();
