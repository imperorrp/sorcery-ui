...
- `CompactNavbar.tsx`: Main navigation bar component with project dropdown, examples section, layout switching, mobile-responsive sheet interface, sliding pill animations, and settings integration. Features Projects card layout for examples, multi-component example descriptions, mobile sheet with settings access, experimental layout restrictions, and differentiated config icon (Settings2). Includes ComponentSwitcher integration for seamless component navigation within projects. Includes comprehensive JSDoc documentation with author attribution. Updated to include a Design Import dialog trigger and additional keyboard shortcuts/UX polish for quick access to the Design Eyedropper flow.
 - `DesignImportDialog.tsx`: Modal interface that uploads a screenshot and invokes the server-side AI pipeline to extract tokens and components for import. Provides a preview of detected components, color palettes, and options to import all generated artifacts into the project. Includes progress/loading state and UX for file selection.
...
- `index.js`, `models/ComponentModel.js`, `routes/componentRoutes.js`: These files are currently empty. Based on the `PLAN.md`, they are intended to provide a future API for saving, loading, and managing user-created components in a MongoDB database.
 ...
 - `test/`: Includes various scripts for testing AI integration (`test-ai-text.js`, `test-genai.js`, `test-generate-object.js`, `test-file-prompt.js`) and an endpoint integration test (`test-endpoint.js`). These scripts are developer-focused and help validate the AI client and server plumbing.
  ...
  - Updated: Several test scripts were added to help with AI integration testing and to speed up local development validation. These scripts demonstrate how to call the server endpoints and verify that the response shapes match `designSystemSchema` where applicable.