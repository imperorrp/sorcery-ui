...
- `main.tsx`: The entry point of the application, responsible for rendering the App component into the DOM. Includes Buffer polyfill for browser compatibility with Babel packages. Sets up NotificationProvider and replaces window.alert with toast notifications.
...
- `renderer.ts`: Contains the crucial `renderCodeToAst` function. This orchestrates the initial code processing pipeline by transpiling the user's code and calling the parser to generate the two distinct visual ASTs (`componentAst` and `componentPreviewAst`). Implements Smart Selection by pruning child components from the preview AST to enforce component boundaries, and handles local component imports with dynamic resolution. Includes automatic mock generation for missing components used in JSX but not imported or available in the library, using global scope injection for reliable component resolution during execution. The renderer also performs defensive sanitization and normalization prior to transpilation (e.g., unwrapping top-level parentheses, removing common export wrapper artifacts, and converting imports to `__deps__` runtime shims) to mitigate parse errors in AI-generated or wrapped code.
- `componentParser.ts`: Handles the serialization from React Elements into our custom `SerializableElement` AST format (`serializeComponent`), and the deserialization from our AST back into renderable React Elements (`renderFromAst`). Includes robust handling of all React children types using `React.Children.toArray()` and wraps custom components in selectable spans with `display: 'contents'` for proper selection behavior.
 - `aiService.ts`: Client-side API wrapper that provides a typed interface to the server AI endpoints for generating design systems from images. Returns `DesignSystemResponse` typed objects for downstream preview/import workflows.
 - `importUtils.ts`: Utilities that convert AI-generated design token objects and Tailwind config objects into strings compatible with the ThemeCssEditor and TailwindConfigEditor, including `jsonCssToCssString` and `jsonConfigToString` helper functions.
...
- `utils.ts`: Contains shared utility functions, such as `cn` for merging Tailwind CSS classes.
 - `runtimeSystem.ts`: Runtime dependency registry that maps import specifiers to actual runtime objects (e.g., `react`, `lucide-react`) to be used by the sandboxed renderer during code execution. Used by the import-to-runtime transform to insulate the iframe environment and avoid bundling issues.
...
- `CompactNavbar.tsx`: Main navigation bar component with project dropdown, examples section, layout switching, mobile-responsive sheet interface, sliding pill animations, and settings integration. Features Projects card layout for examples, multi-component example descriptions, mobile sheet with settings access, experimental layout restrictions, and differentiated config icon (Settings2). Includes ComponentSwitcher integration for seamless component navigation within projects. Includes comprehensive JSDoc documentation with author attribution. Separates "Project" label from project name for clearer UI hierarchy.
 - `DesignImportDialog.tsx`: Modal interface that uploads a screenshot and invokes the server-side AI pipeline to extract tokens and components for import. Provides a preview of detected components, color palettes, and options to import all generated artifacts into the project. Includes progress/loading state and UX for file selection. Uses notifications for user feedback instead of native dialogs.
- `ComponentSwitcher.tsx`: UI component integrated into the navbar for switching between components within the active project. Provides a dropdown interface with search functionality, component creation, and navigation in project-based architecture. Separates "Component" label from component name for clearer UI hierarchy.
...
- `EditorLayout.tsx`: Main editor layout component that manages the entire application interface with panel layout, resizing, fullscreen mode, and floating dock controls. Includes fullscreen mode with automatic panel hiding, floating dock for panel visibility controls, and enhanced visual feedback. Uses notifications for warnings and errors during code application.
...
- `LibraryPanel.tsx`: Comprehensive component library management panel with full CRUD functionality (add, rename, delete, switch between components). Updated to support project-based architecture with editable project names and component list management within projects. Uses notifications with action buttons for confirmations instead of native dialogs.
...
- `notification.tsx`: Notification system with toast UI, provider, and context for displaying user feedback messages with optional actions and details dialog
- `slider.tsx`: Slider component for numeric input with visual feedback and accessibility features

#### `layouts/`

- `ExperimentalLayout.tsx`: The main UI component that assembles the different panels (Library, Navigator, Code Editor, Component Preview, Style Editor). It manages the resizing and collapsing state for these panels using react-resizable-panels. It also triggers the rendering process by calling `renderCodeToAst` and handles example loading. Includes active component selectors for proper multi-component data access, fullscreen mode with automatic panel hiding, floating dock for panel visibility controls, and enhanced visual feedback. Uses notifications for warnings and errors during code application.
- `VibeLayout.tsx`: Alternative layout implementation with floating navigator panel that can be expanded/minimized. Features a compact design with inspector sidebar and floating navigator overlay, including expand/minimize functionality for better space utilization. Includes comprehensive JSDoc documentation with author attribution. Uses notifications for warnings and errors during code application.
- `MobileLayout.tsx`: Touch-optimized layout system for mobile devices that replaces react-resizable-panels with a tabbed/stacked interface. Features bottom tab bar for panel switching, full-screen panels with smooth transitions, swipe gestures for navigation, touch-optimized UI elements, and floating action button for quick actions. Uses notifications for warnings and errors during code application.
...
This directory is set up as a Node.js and Express application used for AI-powered design system extraction and other backend features. The server includes routes for AI processing and example scripts for testing GenAI integration. Note: server environment variables such as `GOOGLE_GENERATIVE_AI_API_KEY` are required for AI features.

- `package.json`: Defines dependencies like express and mongoose.
 - `.env.example`: Contains required development environment variables including `PORT`, `MONGO_URI` and AI keys such as `GOOGLE_GENERATIVE_AI_API_KEY` (copy this to `.env` for local development).
- `index.js`, `models/ComponentModel.js`, `routes/componentRoutes.js`: These files are currently empty. Based on the `PLAN.md`, they are intended to provide a future API for saving, loading, and managing user-created components in a MongoDB database.
 - `index.js`, `models/ComponentModel.js`, `routes/componentRoutes.js`: These files are currently empty or minimal. The `routes/aiRoutes.js` file implements a `/api/ai` endpoint to accept images and return structured design system JSON validated via a Zod schema. It includes normalization and syntax checking of AI responses (strip code fences, normalize font-family, unwrap common wrappers), and responds with a validated design system object suitable for import into the client editor.
 - `lib/designSystemSchema.js`: Contains the Zod schema used by the server to validate AI-generated design system JSON. The schema enforces tokens (cssVars, tailwind config) and component shapes, and the System Prompt is embedded to enforce code style and import rules on the AI side.
 - `lib/db.js`: Database helper that attempts to connect to MongoDB using `MONGO_URI` and `MONGO_DB_NAME`. If DB is unavailable, the server continues to run and logs to disk as a fallback.
 - `lib/db.js`: Database helper that attempts to connect to MongoDB using `MONGO_URI` and `MONGO_DB_NAME`. If DB is unavailable, the server continues to run and logs to disk as a fallback. Includes a documented `connectDB()` function and runtime fallback behavior description.
 - `lib/logger.js`: Server logger that logs to console, an append-only file under `logs/combined.log`, and (optionally) a `Log` MongoDB collection when a DB connection exists. Exposes `requestLogger` middleware for request telemetry.
 - `models/Log.js`: Mongoose model for persisting structured logs. Used by `lib/logger.js` when a DB connection is present.
 - `routes/aiRoutes.js`: Implements the `/api/ai` route and includes the POST `/generate-system` endpoint for parsing, validating, and normalizing AI responses, and a `/status` endpoint for provider checks. It handles inline image data and image URLs, normalizes code fences and font-family tokens, and validates the final structure against the `designSystemSchema`.
 - `test/`: Includes various scripts for testing AI integration (`test-ai-text.js`, `test-genai.js`, `test-generate-object.js`, `test-file-prompt.js`) and an endpoint integration test (`test-endpoint.js`). These scripts are developer-focused and help validate the AI client and server plumbing.
  - Updated: Several test scripts were added to help with AI integration testing and to speed up local development validation. These scripts demonstrate how to call the server endpoints and verify that the response shapes match `designSystemSchema` where applicable. A retired `test-logging.js` remains as a reference for logging pipeline testing and is intentionally disabled to avoid accidental execution.
  - Planned: Add automated unit tests for server normalization (`normalizeDesignSystem`) and client renderer normalization (unwrapping and export wrapper removal), plus mock AI integration tests to automate the `generate-system` endpoint validation in CI.